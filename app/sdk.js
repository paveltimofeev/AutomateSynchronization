window.PDK || (function (window) {
	function copy(a, b, c, d) {
		for (var e in b)
			(c || "undefined" == typeof a[e]) && (a[e] = d ? d(b[e]) : b[e]);
		return a
	}
	function create(a, b) {
	...
	include(PDK, {
		me : function () {
			...
			d = c.shift();
			d && "string" == typeof d ? a = d : "function" == typeof d && c.unshift(d),
			a && (b += 0 === a.indexOf("/") ? a.substr(1) : a),
			c.unshift(b),
			PDK.request.apply(void 0, c)
		},
		request : function () {
			var a,
			b,
			...
			for ("/" === e[0] && (e = e.substr(1)), -1 === e.indexOf(Api.__PREFIX) && (e = Api.__PREFIX + e), -1 === e.indexOf("/", e.length - 1) && (e += "/"); f; ) {
				...
				if ("string" !== g || a)
					if ("function" !== g || c) {
						if ("object" !== g || b)
							return void log("Invalid argument passed to api:" + f);
						b = f
					} else
						c = f;
				else
	...
	var Api = {
		...
		triggerRequest : function (a, b, c, d, e, f) {
			var g = f || c;
			window.XMLHttpRequest ? a.open(b, g, !0) : window.XDomainRequest && a.open(b, g),
			a.setRequestHeader(Api.__AUTHORIZATION_HEADER, "BEARER " + e),
			a.send(d)
		},
		executeCorsRequest : function (a, b, c, d, e, f, g) {
			...
			k = _domains.api + a;
			j && (k += (a.indexOf("?") > -1 ? "&" : "?") + j),
			window.XMLHttpRequest ? (h = new window.XMLHttpRequest, "withCredentials" in h && (h.onerror = g, h.onreadystatechange = function () {
		...
		invokeXHRCallback : function (a, b, c, d, e, f, g) {
			...
			if (f = f || function () {}, g)
				...
				try {
					j = JSON.parse(a.responseText) || j,
					h = j.page,
					h && h.next && (i = h.next, j.hasNext = !0),
					j.hasNext && (j.next = function () {
				...
				j.error = "The request did not return a valid response";
	...
	include(PDK, {
		login : function (a, b) {
			...
			if (a.scope) {
				...
				c = "Cannot login without providing scope as an option";
			return c ? void b({
				error : c
			}) : (a = copy({
						method : "auth.login"
		...
		logout : function (a) {
			...
			b.session && (b.error = "The active users session was not cleared."),
	...
	var Auth = {
		...
		setSession : function (a) {
			...
			return _session = a,
			d && Cookie.isEnabled() && Cookie.set(a),
			e
		},
	...
	var UI = {
		...
		createCall : function (a, b) {
			...
			if (!c.transform || (e = c.transform(e))) {
				...
				e.id in UI._defaultCb || (UI._defaultCb[e.id] = e.cb);
		...
		popup : function (a) {
			var b,
			c = "undefined" != typeof window.screenX ? window.screenX : window.screenLeft,
			...
			l = window.location,
			m = !1;
			l && (l = l.origin || l.protocol + "//" + l.host),
			UI._active[a.id] = {
			...
			b = function (b) {
				if (!m) {
					...
					window.removeEventListener("message", arguments.callee, !1),
					UI.triggerEvent(a.id, b.data),
					m = !0
				}
			},
	...
	UI.Methods = {
		"auth.login" : {
			...
			transform : function (a) {
				...
				if (b)
					return a.useRedirect = !window.postMessage || /(iPhone|iPod|iPad).*AppleWebKit(?!.*Version)/i.test(navigator.userAgent) || -1 !== navigator.userAgent.indexOf("MSIE") || navigator.appVersion.indexOf("Trident/") > 0, UI._defaultCb[a.id] = function (c) {
						var d = {};
						if (c && c.access_token) {
							var e = {
		...
		pin : {
			...
			transform : function (a) {
				return a.url = _domains.www + this.url,
				a.useRedirect = !0,
				a
			}
		}
	...
	var Cookie = {
		...
		load : function () {
			var a,
			b = document.cookie.match("\\bps_" + _appId + '="([^;]*)\\b');
			return b && (a = QS.decode(b[1]), Cookie._domain = a.base_domain),
			a
		},
		setRaw : function (a, b, c) {
			document.cookie = "ps_" + _appId + '="' + a + '"' + (a && 0 === b ? "" : "; expires=" + new Date(1e3 * b).toGMTString()) + "; path=/" + (c ? "; domain=." + c : ""),
			Cookie._domain = c
		},
		set : function (a) {
